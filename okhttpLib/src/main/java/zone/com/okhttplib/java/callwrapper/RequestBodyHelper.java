...
import zone.com.okhttplib.java.callwrapper.core.MediaTypeUtils;
import zone.com.okhttplib.java.callwrapper.core.ProgressCallback;
import zone.com.okhttplib.java.callwrapper.upload.ProgressRequestBody;
    ...
    public RequestBody createRequestBody(ProgressCallback listener) {
        ...
        if (getFileMap() == null && getFileNameMap() == null) {
            ...
            for (Map.Entry<String, File> item : getFileMap().entrySet()) {
                MediaType parse = MediaType.parse(MediaTypeUtils.getFileSuffix(item.getValue()));
                RequestBody body = RequestBody.create(parse, item.getValue());
                form.addFormDataPart(item.getKey(), getFileNameMap().get(item.getKey()), body);
            }
